...
export default function Post( { wID, post, user, comments, alerts, alertsDispatch } )  {
    ...
    useEffect(async () => {
        ...
        setWhData(await WateringHoles.getWateringHole(wID));
    }, [])
        ...
        <>
            <div className='py-24 flex flex-col'>
                <div className='flex relative justify-center'>
                    ...
                    <div className='w-2/3 bg-blue-600 rounded-lg shadow-2xl border-yellow-400 border-2 flex flex-col items-center my-20'>
                ...
                <div className='flex flex-col justify-around'>
                {
                    parsedComments.map(function( comment ) {
                        return (
                            <Comment key={comment} data={comment} postID={parseInt(parsedPost[0]._hex, 16)} alerts={alerts} alertsDispatch={alertsDispatch} />
                        )
                    })
                }
                </div>
            ...
            <div className='fixed z-40 right-1 bottom-20'>
                <div className='text-yellow-400'>
                    <AddButton type={0} wID={wID} pID={parseInt(parsedPost[0]._hex, 16)} />
                </div>
            </div>
        </>
    )
