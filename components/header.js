...
const Header = ({ overlayVisible, setOverlayVisible }) => {
    ...
    return (
        <div className='flex flex-row fixed top-0 min-w-full justify-between bg-purple-600 p-6 z-30'>
            <Link href='/'>
                <h1 className='hover:text-yellow-100 ml-4 font-holocene text-2xl text-yellow-400 cursor-pointer p-2'>Dilettante</h1>
            </Link>
            {
                !wallet.account ? 
                <div>
                    <button onClick={() => setOverlayVisible(!overlayVisible)} className='p-2 flex flex-row rounded cursor-pointer hover:bg-yellow-300 active:bg-yellow-200 bg-yellow-400 mt-1 mr-4'>
